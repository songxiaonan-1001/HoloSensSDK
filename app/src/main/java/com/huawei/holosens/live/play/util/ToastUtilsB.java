...
public class ToastUtilsB {
    ...
    public static void show(@StringRes final int resId) {
        show(Utils.getApp().getResources().getString(resId));
    }

    public static void show(String message) {
        Log.e("aslalksdasf", "show: " + message );
        setGravity(Gravity.CENTER, -1, -1);
        View layout = LayoutInflater.from(Utils.getApp()).inflate(R.layout.dialog_toast, null);
        TextView tip = layout.findViewById(R.id.tv_tip);
        tip.setText(message);
        showCustomShort(layout);
    }
